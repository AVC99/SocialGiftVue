<template>
    <div class="flex flex-row  justify-between items-center h-auto space-x-3 bg-tertiary rounded-2xl p-2">
        ...
        <button @click="giftProduct" class=" items-center rounded-full bg-primary h-10 w-10">
...
export default {
    ...
    methods: {
        async giftProduct() {
            try {
                await bookGift(this.product.id).then(() => {
                    location.reload();
                    alert("Product gifted!");
                });
            } catch (error) {
                console.log(error);
            }
